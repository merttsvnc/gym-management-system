...
if [[ ! -d ".git" ]]; then
  err "Repo root'ta .git bulunamadı. Bu script repo root'ta çalışmalı."
  exit 1
fi

# Network hazırla (yoksa oluştur)
if ! sudo docker network inspect "$NETWORK_NAME" >/dev/null 2>&1; then
  log "Network '${NETWORK_NAME}' yok, oluşturuluyor..."
  sudo docker network create "$NETWORK_NAME" >/dev/null
fi

# backend env hazırlığı (compose interpolation için .env gerekir)
ensure_backend_env() {
  if [[ ! -f "${BACKEND_DIR}/.env.prod" ]]; then
    err "${BACKEND_DIR}/.env.prod yok. Prod env dosyasını oluştur."
    exit 1
  fi

  if [[ ! -f "${BACKEND_DIR}/.env" ]]; then
    log "backend/.env yok → backend/.env.prod kopyalanıyor (compose interpolation için)"
    cp "${BACKEND_DIR}/.env.prod" "${BACKEND_DIR}/.env"
  fi
}

current_sha()  { git rev-parse --short HEAD; }
upstream_sha() {
  git ls-remote origin -h "refs/heads/${REPO_BRANCH}" | awk '{print $1}' | cut -c1-7
}
...
build_and_deploy() {
  ...
  log "Compose build (no-cache) + up..."
  # Eski stack'i indirip temiz build almak, prisma generate/build sorunlarını cache'ten kaçırmamak için.
  ...
  if ! health_check; then
    err "Health-check başarısız. Son loglar:"
    sudo docker logs --tail 200 "${API_CONTAINER}" || true
    err "Traefik son loglar:"
    sudo docker logs --tail 200 traefik || true
    exit 1
  fi

  log "Temizlik: dangling image prune"
...
case "${1:-once}" in
  once)
    if has_updates; then
      log "Uzakta yeni commit var → deploy başlıyor"
      build_and_deploy
    else
      log "Uzakta yeni commit yok. Yine de yeniden deploy etmek için: $0 force"
    fi
    ;;
  force)
    log "Force mod: yeniden build + deploy"
    build_and_deploy
    ;;
  watch)
    log "Watch mod: origin/${REPO_BRANCH} yeni commit aldıkça otomatik deploy"
    while true; do
      if has_updates; then
        log "Değişiklik tespit edildi → deploy"
        build_and_deploy
      fi
      sleep 20
    done
