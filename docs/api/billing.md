...
- **Optional legacy bridge:** when `BILLING_LEGACY_FALLBACK_ENABLED=true`, tenants without a RevenueCat entitlement snapshot may still get premium if `billingStatus` is `ACTIVE`. **`TRIAL` no longer grants access on this path** — backend-managed trial is deprecated and free trials are exclusively provided via StoreKit / RevenueCat introductory offers. Keep `false` in production (see `backend/.env.example`).
