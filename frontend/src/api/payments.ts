...
export async function createPayment(
  ...
  try {
    ...
    if (apiError.statusCode === 400) {
      throw new Error(apiError.message || 'Ödeme oluşturulurken doğrulama hatası oluştu');
    }
    ...
    if (apiError.statusCode === 429) {
      throw new Error(
        apiError.message || 'Çok fazla istek gönderildi. Lütfen birkaç dakika sonra tekrar deneyin.',
      );
    }
...
export async function getPayments(
  ...
  if (queryParams.includeCorrections !== undefined) {
    searchParams.append('includeCorrections', queryParams.includeCorrections.toString());
  }
  ...
  const url = `/payments${searchParams.toString() ? `?${searchParams.toString()}` : ''}`;
...
export async function getMemberPayments(
  ...
  const url = `/members/${memberId}/payments${searchParams.toString() ? `?${searchParams.toString()}` : ''}`;
...
export async function correctPayment(
  ...
  try {
    ...
    if (apiError.statusCode === 400) {
      ...
      if (message.includes('zaten düzeltilmiş') || message.includes('already corrected')) {
        throw new Error(
          apiError.message || 'Bu ödeme zaten düzeltilmiş. Bir ödeme yalnızca bir kez düzeltilebilir.',
        );
      }
      throw new Error(apiError.message || 'Ödeme düzeltilirken doğrulama hatası oluştu');
    }
    ...
    if (apiError.statusCode === 409) {
      throw new Error(
        apiError.message || 'Ödeme başka bir kullanıcı tarafından güncellenmiş. Lütfen sayfayı yenileyip tekrar deneyin.',
      );
    }
